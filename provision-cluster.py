...
parser.add_argument('--cluster-name', required=False,
                    help="a user denote name of the cluster")
...
try:
    ...
    output_file = os.environ.get('GITHUB_OUTPUT') if os.environ.get(
        'GITHUB_OUTPUT') else 'clusterId.txt'
    ...
    while hres_data['stage'] != 'k8sReady' or hres_data['stageIndex'] < 9:
        ...
        print(
            f'cluster {clusterId} stage {int(round((hres_data["stageIndex"]/9)*100,0))}%')
